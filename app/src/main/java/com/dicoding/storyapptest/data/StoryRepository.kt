...
import com.dicoding.storyapptest.data.response.ErrorResponse
import com.dicoding.storyapptest.data.response.UploadFileResponse
import com.dicoding.storyapptest.data.retrofit.ApiService
import com.dicoding.storyapptest.data.room.StoryDao
...
class StoryRepository private constructor(
    ...
    fun uploadStories(imageFile: File, description: String) = liveData {
        try {
            ...
            val jsonInString = e.response()?.errorBody()?.string()
            val errorBody = Gson().fromJson(jsonInString, UploadFileResponse::class.java)
            val errorMessage = errorBody.message
            emit(errorMessage?.let { Result.Error(it) })
        }
    }
